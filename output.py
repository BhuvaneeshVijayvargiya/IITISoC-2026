...
def write_single(result: dict, out_dir: str = "output"):
    ...
    write_json(
        result,
        os.path.join(out_dir, "solution.json")
    )

    write_placements_csv(
        result,
        os.path.join(out_dir, "placements.csv")
    )

    write_uld_stats_csv(
        result,
        os.path.join(out_dir, "uld_stats.csv")
    )

    write_unpacked_csv(
        result,
        os.path.join(out_dir, "unpacked.csv")
    )
...
def write_multi(all_results: list, out_dir: str = "output"):
    ...
    if feasible:
        ...
        best = min(
            feasible,
            key=lambda r: r["summary"]["total_cost"]
        )

        write_official_txt(
            best,
            os.path.join(out_dir, "solution_best.txt")
        )

        write_json(
            best,
            os.path.join(out_dir, "solution_best.json")
        )

        print(
            f"[OK] solution_best.txt -> {out_dir}/solution_best.txt "
            f"(run {best['run_id']}, "
            f"cost={best['summary']['total_cost']})"
        )

    with open(
        os.path.join(out_dir, "all_runs.json"),
        "w"
    ) as f:
    ...
    print(
        f"[OK] all_runs.json -> {out_dir}/all_runs.json"
    )
    ...
    for r in all_results:
        ...
        write_official_txt(
            ...
            os.path.join(
                runs_dir,
                f"solution_run{r['run_id']:04d}.txt"
            )
        )

    print(
        f"[OK] per-run txt files -> {runs_dir}"
    )
    ...
    print(
        f"[OK] all_placements.csv -> {csv_path}"
    )
    ...
    with open(summary_path, "w", newline="") as f:
        ...
        for r in all_results:
            ...
            w.writerow({
                ...
                "priority_failures": ";".join(
                    s["priority_failures"]
                ),
            })

    print(
        f"[OK] runs_summary.csv -> {summary_path}"
    )
